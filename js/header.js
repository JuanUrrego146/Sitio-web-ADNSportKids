(function () {
  ...
  function ensureUI(root) {
    ...
    const iconBtn = (aria, d, stroke = false) => {
      ...
      const key = aria.toLowerCase();
      if (key === 'favoritos') a.href = 'favoritos.html';
      else if (key === 'cuenta') a.href = 'registro.html';
      else if (key === 'carrito') a.href = 'Carrito.html';
      else a.href = `#${key}`;
    ...
    if (!root.querySelector('.adn-icon-btn[aria-label="Favoritos"]')) {
      actions.appendChild(
        iconBtn(
          'Favoritos',
          'M12 21c-.2 0-6.1-4.3-8.5-7.2C1.9 11.9 2.2 9 4.5 7.8c1.6-.9 3.6-.6 4.9.6L12 10.9l2.6-2.5c1.3-1.2 3.3-1.5 5-.6 2.3 1.2 2.6 4.1.5 6-2.4 2.9-8.3 7.2-8.5 7.2z'
        )
      );
    }
    
    const favBtn = root.querySelector('.adn-icon-btn[aria-label="Favoritos"]');
    if (favBtn) favBtn.href = 'favoritos.html';

    
    if (!root.querySelector('.adn-search')) {
      const form = document.createElement('form');
      form.className = 'adn-search';
      ...
      Object.assign(input, { type: 'search', placeholder: 'Buscar...' });
